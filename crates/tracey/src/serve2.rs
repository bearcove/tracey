//! HTTP server for the tracey dashboard (v2)
//!
...
#![allow(dead_code)] // API types are constructed via JSON serialization
...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:wght@400;500&family=Public+Sans:wght@400;500;600&display=swap" rel="stylesheet">
  <script type="importmap">
  {
    "imports": {
      ...
      "@arborium/arborium": "https://esm.sh/@arborium/arborium@2.4.6",
...
<body>
  ...
  <script type="module">
        ...
        async function poll() {
          try {
            ...
            if (lastVersion !== null && data.version !== lastVersion) {
              // Reload the page on version change
    ...
    function App() {
      ...
      if (error) {
        return html`<div class="loading">Error: ${error}</div>`;
      }

      if (!config || !forward || !reverse) {
      ...
      return html`
        <div style="max-width: 1400px; margin: 0 auto; padding: 2rem;">
          <header style="margin-bottom: 2rem;">
            <h1 style="margin: 0 0 1rem; font-size: 1.5rem;">tracey</h1>
            <nav style="display: flex; gap: 1rem;">
              <button
                onClick=${() => setView('forward')}
                style="padding: 0.5rem 1rem; background: ${view === 'forward' ? 'var(--accent)' : 'transparent'}; color: ${view === 'forward' ? 'white' : 'var(--fg)'}; border: 1px solid var(--border); border-radius: 6px; cursor: pointer;"
              >Forward</button>
              <button
                onClick=${() => setView('reverse')}
                style="padding: 0.5rem 1rem; background: ${view === 'reverse' ? 'var(--accent)' : 'transparent'}; color: ${view === 'reverse' ? 'white' : 'var(--fg)'}; border: 1px solid var(--border); border-radius: 6px; cursor: pointer;"
              >Reverse</button>
            </nav>
          </header>

          ${view === 'forward' && html`<${ForwardView} data=${forward} />`}
          ${view === 'reverse' && html`<${ReverseView} data=${reverse} config=${config} />`}
        </div>
      `;
    }

    // Forward Traceability View
    function ForwardView({ data }) {
      const allRules = data.specs.flatMap(s => s.rules.map(r => ({ ...r, spec: s.name })));
      const totalRules = allRules.length;
      const implCovered = allRules.filter(r => r.implRefs.length > 0).length;
      const verifyCovered = allRules.filter(r => r.verifyRefs.length > 0).length;

      return html`
        <div>
          <div style="display: flex; gap: 2rem; margin-bottom: 1.5rem;">
            <div>
              <div style="font-size: 0.85rem; color: var(--fg-muted);">Total Rules</div>
              <div style="font-size: 1.5rem; font-weight: 600;">${totalRules}</div>
            </div>
            <div>
              <div style="font-size: 0.85rem; color: var(--fg-muted);">Impl Coverage</div>
              <div style="font-size: 1.5rem; font-weight: 600; color: var(--green);">${totalRules ? ((implCovered / totalRules) * 100).toFixed(1) : 0}%</div>
            </div>
            <div>
              <div style="font-size: 0.85rem; color: var(--fg-muted);">Test Coverage</div>
              <div style="font-size: 1.5rem; font-weight: 600; color: var(--accent);">${totalRules ? ((verifyCovered / totalRules) * 100).toFixed(1) : 0}%</div>
            </div>
          </div>
          ...
          <table style="width: 100%; border-collapse: collapse; font-family: 'IBM Plex Mono', monospace; font-size: 0.9rem;">
            <thead>
              <tr style="border-bottom: 1px solid var(--border);">
                <th style="text-align: left; padding: 0.75rem;">Rule</th>
                <th style="text-align: left; padding: 0.75rem;">Impl</th>
                <th style="text-align: left; padding: 0.75rem;">Verify</th>
              </tr>
            </thead>
            <tbody>
              ${allRules.map(rule => html`
                <tr key=${rule.id} style="border-bottom: 1px solid var(--border);">
                  <td style="padding: 0.75rem; vertical-align: top;">
                    <div style="font-weight: 500;">${rule.id}</div>
                    ${rule.text && html`<div style="font-family: 'Public Sans', sans-serif; color: var(--fg-muted); margin-top: 0.25rem; font-size: 0.85rem;">${rule.text}</div>`}
                  </td>
                  <td style="padding: 0.75rem; vertical-align: top;">
                    ${rule.implRefs.length > 0
                      ? rule.implRefs.map(r => html`<div style="color: var(--green);">${r.file}:${r.line}</div>`)
                      : html`<span style="color: var(--fg-muted);">-</span>`
                    }
                  </td>
                  <td style="padding: 0.75rem; vertical-align: top;">
                    ${rule.verifyRefs.length > 0
                      ? rule.verifyRefs.map(r => html`<div style="color: var(--accent);">${r.file}:${r.line}</div>`)
                      : html`<span style="color: var(--fg-muted);">-</span>`
                    }
                  </td>
                </tr>
    ...
    // Reverse Traceability View
    function ReverseView({ data, config }) {
      const [selectedFile, setSelectedFile] = useState(null);
      const [fileData, setFileData] = useState(null);

      useEffect(() => {
        if (selectedFile) {
          fetchJson('/api/file?path=' + encodeURIComponent(selectedFile))
            .then(setFileData)
            .catch(e => console.error('Failed to load file:', e));
        }
      }, [selectedFile]);

      const pct = data.totalUnits ? ((data.coveredUnits / data.totalUnits) * 100).toFixed(1) : 0;
        ...
        <div style="display: grid; grid-template-columns: 300px 1fr; gap: 1rem; height: calc(100vh - 200px);">
          <div style="border: 1px solid var(--border); border-radius: 8px; overflow: auto;">
            <div style="padding: 1rem; border-bottom: 1px solid var(--border);">
              <div style="font-size: 0.85rem; color: var(--fg-muted);">Code Coverage</div>
              <div style="font-size: 1.25rem; font-weight: 600;">${pct}%</div>
              <div style="font-size: 0.75rem; color: var(--fg-muted);">${data.coveredUnits} / ${data.totalUnits} units</div>
            </div>
            <div style="padding: 0.5rem;">
              ${data.files.map(f => {
                const filePct = f.totalUnits ? ((f.coveredUnits / f.totalUnits) * 100).toFixed(0) : 0;
                const isSelected = selectedFile === f.path;
                return html`
                  <div
                    key=${f.path}
                    onClick=${() => setSelectedFile(f.path)}
                    style="padding: 0.5rem; cursor: pointer; border-radius: 4px; background: ${isSelected ? 'var(--hover)' : 'transparent'}; display: flex; justify-content: space-between; align-items: center;"
                  >
                    <span style="font-size: 0.8rem; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${f.path}</span>
                    <span style="font-size: 0.75rem; color: ${f.coveredUnits === f.totalUnits ? 'var(--green)' : 'var(--fg-muted)'};">${filePct}%</span>
                  </div>
                `;
              })}
            </div>
          ...
          <div style="border: 1px solid var(--border); border-radius: 8px; overflow: auto;">
            ${selectedFile && fileData
              ? html`<${FileView} data=${fileData} />`
              : html`<div style="padding: 2rem; color: var(--fg-muted); text-align: center;">Select a file to view coverage</div>`
            }
          </div>
        </div>
      `;
    ...
    function FileView({ data }) {
      ...
      for (const unit of data.units) {
        for (let line = unit.startLine; line <= unit.endLine; line++) {
          if (!lineAnnotations.has(line)) {
            lineAnnotations.set(line, { units: [], covered: false });
          }
          const anno = lineAnnotations.get(line);
          anno.units.push(unit);
          if (unit.ruleRefs.length > 0) {
            anno.covered = true;
          }
        }
      }
      ...
      return html`
        <div style="font-family: 'IBM Plex Mono', monospace; font-size: 0.85rem;">
          <div style="padding: 0.5rem 1rem; border-bottom: 1px solid var(--border); font-weight: 500;">
            ${data.path}
          </div>
          <pre style="margin: 0; padding: 0; overflow: auto;">
            ${lines.map((line, i) => {
              ...
              const bgColor = anno
                ? anno.covered ? 'rgba(22, 163, 74, 0.1)' : 'rgba(220, 38, 38, 0.1)'
                : 'transparent';

              return html`
                <div key=${lineNum} style="display: flex; background: ${bgColor};">
                  <span style="width: 4rem; text-align: right; padding-right: 1rem; color: var(--fg-muted); user-select: none;">${lineNum}</span>
                  <span style="flex: 1; padding-right: 1rem; white-space: pre;">${line || ' '}</span>
                </div>
              `;
            })}
          </pre>
