...
fn highlight_simple(content: &str, query: &str) -> String {
    ...
    if content.is_ascii() && query.is_ascii() {
        // ASCII-only: lowercasing preserves byte offsets, so indices from the
        // lowercased string are valid into the original.
        let content_lower = content.to_lowercase();
        let query_lower = query.to_lowercase();
        for (start, m) in content_lower.match_indices(&query_lower) {
            result.push_str(&content[last_end..start]);
            result.push(MARK_OPEN);
            result.push_str(&content[start..start + m.len()]);
            result.push(MARK_CLOSE);
            last_end = start + m.len();
        }
    } else {
        // Non-ASCII: to_lowercase can change byte length (İ, ß), so offsets
        // from the lowercased string can't index the original safely. Fall
        // back to case-sensitive matching on the original — correct Unicode
        // case-folding is out of scope for this simple fallback.
        for (start, m) in content.match_indices(query) {
            result.push_str(&content[last_end..start]);
            result.push(MARK_OPEN);
            result.push_str(m);
            result.push(MARK_CLOSE);
