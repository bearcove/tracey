...
pub async fn run(project_root: PathBuf, config_path: PathBuf) -> Result<()> {
    ...
    tokio::spawn(async move {
        ...
        // Compile include/exclude globs once up front; rebuilt only on
        // `Reconfigure` so the per-event hot path is a cheap `is_match`.
        let (mut include_set, mut include_configured, mut exclude_set) =
            build_watch_globsets(&config_path_for_rebuild, &engine_for_rebuild).await;
        ...
        while let Some(event) = watcher_rx.recv().await {
            match event {
                WatcherEvent::Reconfigure => {
                    ...
                    (include_set, include_configured, exclude_set) =
                        build_watch_globsets(&config_path_for_rebuild, &engine_for_rebuild).await;
    ...
    #[cfg(unix)]
    let listener = LocalLinkAcceptor::bind(&endpoint)
...
///    r[impl server.watch.patterns-from-config]
///
/// `include_configured` is `true` when at least one include pattern was
/// *configured* (regardless of whether it compiled). An invalid pattern
/// must fail closed (reject-all), not silently degrade to accept-all.
///
...
/// individual patterns are skipped (matching the previous per-call
/// `Glob::new(..).ok()` behaviour); a build failure falls back to an empty
/// set.
...
/// Collect every spec/impl `include` + `exclude` pattern from the on-disk
/// config and compile them into `(include_set, include_configured, exclude_set)`.
/// `include_configured` records whether *any* include patterns were configured
/// (so an all-invalid include list fails closed). Falls back to the engine's
/// cached config (spec source paths only) when the config file can't be
/// re-read.
async fn build_watch_globsets(
    config_path: &PathBuf,
    engine: &Arc<Engine>,
) -> (GlobSet, bool, GlobSet) {
    let mut include: Vec<String> = Vec::new();
    let mut exclude: Vec<String> = Vec::new();
    if let Ok(config) = crate::load_config(config_path) {
        for spec in &config.specs {
            include.extend(spec.include.iter().cloned());
            for impl_ in &spec.impls {
                include.extend(impl_.include.iter().cloned());
                exclude.extend(impl_.exclude.iter().cloned());
            }
        }
    } else {
    ...
    let include_configured = !include.is_empty();
    (build_globset(&include), include_configured, build_globset(&exclude))
}
