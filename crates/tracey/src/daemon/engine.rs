...
pub struct Engine {
    ...
    spec_file_deps: Arc<RwLock<Arc<HashSet<PathBuf>>>>,
...
impl Engine {
    ...
    pub async fn new(project_root: PathBuf, config_path: PathBuf) -> Result<Self> {
        ...
        let engine = Self {
            ...
            search_reindex_tx,
            search_activated,
            spec_file_deps: Arc::new(RwLock::new(Arc::new(HashSet::new()))),
    ...
    ///
    /// Returns a cheap `Arc` clone of the current set; callers see a
    /// consistent snapshot without copying every `PathBuf` per watcher tick.
    pub async fn spec_file_deps(&self) -> Arc<HashSet<PathBuf>> {
    ...
    pub async fn record_spec_file_deps(&self, deps: HashSet<PathBuf>) {
        ...
        let mut guard = self.spec_file_deps.write().await;
        Arc::make_mut(&mut guard).extend(deps);
    }
