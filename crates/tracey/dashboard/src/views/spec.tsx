...
function isActiveOrHasActiveChild(
	...
	return node.children.some((child) =>
		isActiveOrHasActiveChild(child, activeHeading),
	);
}
...
function OutlineTree({
    ...
    ${nodes.map((node) => {
			...
			const showCoverage =
				h.aggregated.total > 0 && (!hasChildren || isCollapsed);
...
export function SpecView({
	...
	useEffect(() => {
		if (!contentRef.current || !contentBodyRef.current || outline.length === 0)
			return;
	...
	useEffect(() => {
		...
		for (const el of elements) {
			...
			const fullPath = config.projectRoot
				? `${config.projectRoot}/${sourceFile}`
				: sourceFile;
			const editUrl = EDITORS.zed.urlTemplate(
				fullPath,
				parseInt(sourceLine, 10),
			);
	...
	useEffect(() => {
		...
		const handleClick = (e: Event) => {
			...
			const ruleMarker = target.closest(
			...
			const refBadge = target.closest(
				"a.rule-badge[data-all-refs]",
			) as HTMLElement | null;
			if (refBadge) {
	...
	useEffect(() => {
		...
		requestAnimationFrame(() => {
			...
			requestAnimationFrame(() => {
				...
				if (selectedRule) {
					const ruleEl = contentRef.current.querySelector(
						`[data-rule="${selectedRule}"]`,
					);
					if (ruleEl) {
						const containerRect =
							contentBodyRef.current.getBoundingClientRect();
						const ruleRect = ruleEl.getBoundingClientRect();
						const currentScroll = contentBodyRef.current.scrollTop;
						const targetScrollTop =
							currentScroll + (ruleRect.top - containerRect.top) - 150;
					...
					selectedHeading &&
					selectedHeading !== lastScrolledHeading.current
				) {
