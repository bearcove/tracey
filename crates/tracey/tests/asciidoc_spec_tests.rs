...
async fn test_adoc_fixture_has_table_reqs() {
    ...
    assert!(
        ids.contains(&"table.structure".to_string()),
        "table.structure should be present, got: {:?}",
        ids
    );
    assert!(
        ids.contains(&"table.inline-code".to_string()),
        "table.inline-code should be present"
    );
    assert!(
        ids.contains(&"format.bold".to_string()),
        "format.bold should be present"
    );
    assert!(
        ids.contains(&"format.multiline".to_string()),
        "format.multiline should be present"
    );
}
...
async fn test_adoc_multiline_req_raw() {
    ...
    let req = doc
        .reqs
        .iter()
        .find(|r| r.id.to_string() == "multi.line")
        .expect("multi.line");
    assert!(
...
async fn test_adoc_cross_file_xref_does_not_error() {
    ...
    let doc = parse_spec(SpecFormat::AsciiDoc, src)
        .await
