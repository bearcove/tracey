...
impl<B: SpecBackend> DynBackend for B {
    ...
    async fn render_html(
        ...
        let cfg = cfg
            ...
            .expect("config type mismatch: did SpecConfigs::insert use the wrong backend?");
...
impl SpecConfigs {
    /// Override the config for backend `B`.
    ///
    /// The format key is derived from `B::format`, so a `cfg` of the wrong type
    /// for a given format is unrepresentable.
    pub fn insert<B: SpecBackend + Default>(&mut self, cfg: B::Config) {
        self.0.insert(B::default().format(), ErasedConfig::new(cfg));
    }
