...
//! `inline_code_spans`. The `html` field is left empty; the full
//! typst→HTML compiler in [`render_display`] fills it behind a feature gate.
//!
...
use super::{
    BadgeFn, RenderInput, RenderOutput, RenderedSection, SlugAllocator, SpecBackend, SpecDoc,
    SpecFormat,
};
...
impl SpecBackend for Typst {
    ...
    async fn render_html(
        ...
        let RenderInput {
            ...
            deps,
            badge_for,
            // inline_code unused: typst has its own renderer
            ..
        } = input;

        let mut sections = Vec::with_capacity(sources.len());
        // Capture the first error rather than `?`-propagating so later sources
        // still run and record their `#import` deps — the watcher must learn
        // about *every* source's helpers even if an earlier one fails.
        let mut first_err: Option<eyre::Error> = None;
        for (idx, src) in sources.iter().enumerate() {
            let abs_source = root.join(src.path);
            match render_display(
                src.content,
                &abs_source,
                root,
        ...
        // Clear typst's global memoization cache once per batch so a
        // long-running daemon doesn't accumulate unbounded memory, while still
        // letting files within the same batch share cached subcomputations.
        // Runs unconditionally so an error mid-batch doesn't leave the cache
        // populated.
...
///    badge markup from `badge_for` (open) + [`super::REQ_CONTAINER_CLOSE`], and
///    rewrite each heading sentinel to `<hN id="slug">` using the slugified
///    `data-base-slug`. `headings` and `elements` are rebuilt from sentinel
///    order so headings from `#heading(..)` calls or `#include`d files appear
///    in the outline.
/// 4. Lift `<style>` / `<link>` from the compiler's `<head>` into
///    `head_injections`; return only the `<body>` interior as `html`.
///
/// `source_path` is the path to the spec file itself. `root` is the tracey
/// project root and becomes the typst World root, so relative `#import` /
/// `#include` paths may reach any file under the project (e.g.
/// `"../sibling/helper.typ"` from a spec in a subdirectory). The main module's
/// virtual path is `source_path` relative to `root`, so a sibling file with the
/// same name is not shadowed. Package imports (`@preview/...`) other than the tracey shim
...
pub async fn render_display(
    ...
    #[cfg(feature = "typst-spec")]
    {
        compiler::render(content, source_path, root, package_path, badge_for, alloc, deps).await
    }
}
...
fn extract_req(code: Node<'_>, bytes: &[u8]) -> Option<ReqDefinition> {
    ...
    // glossarium's `#gls`, etc.) define their own functions that take string
    // args, and every such call would otherwise be picked up as a phantom
    // marker and poison downstream prefix inference.
...
#[cfg(feature = "typst-spec")]
mod compiler {
    use super::super::REQ_CONTAINER_CLOSE;
    use super::{BadgeFn, SlugAllocator, SpecDoc, parse};
    ...
    /// Prelude prepended to every spec before compilation.
    ///
    ...
    pub(super) async fn render(
        ...
        let world = SpecWorld::new(full, source_path, root, package_path.map(Path::to_path_buf));
        // `typst::compile` is CPU-bound and `SpecWorld::file()`/`source()` do
        // sync `std::fs::read`; running it inline would pin a tokio worker for
        // the whole compile. `spawn_blocking` (not `block_in_place`) so the
        // current-thread runtime used by `#[tokio::test]` keeps working.
        // `world` round-trips through the closure so `read_paths` can be
        // drained afterwards.
        let (compiled, world) = tokio::task::spawn_blocking(move || {
            let compiled = typst::compile::<HtmlDocument>(&world);
            (compiled, world)
        })
        .await
        .expect("typst compile task panicked");
        ...
        // `pretty: true` matches typst-html 0.14's hardcoded behaviour; the
        // post-processing scanners below are whitespace-tolerant either way.
        let html = typst_html::html(&output, &HtmlOptions { pretty: true })
        ...
        doc.html = splice_req_badges(&body, &by_id, badge_for, &source_path_str);
    ...
    /// probe order.
    /// Process-global typst library + embedded font set. Both are pure
    /// functions of compile-time constants, so building them once per process
    /// (instead of once per [`SpecWorld`]) avoids re-walking the embedded font
    /// blobs on every spec file.
    static TYPST_GLOBALS: OnceLock<(LazyHash<Library>, FontStore)> = OnceLock::new();

    fn typst_globals() -> &'static (LazyHash<Library>, FontStore) {
        TYPST_GLOBALS.get_or_init(|| {
            let features: Features = [Feature::Html].into_iter().collect();
            let library = Library::builder().with_features(features).build();
            // typst-kit 0.15 replaced `Fonts::searcher()` with a `FontStore`
            // fed from free-function font providers; we only need embedded.
            let mut fonts = FontStore::new();
            fonts.extend(typst_kit::fonts::embedded());
            (LazyHash::new(library), fonts)
        })
    }
    ...
    impl SpecWorld {
        fn new(text: String, source_path: &Path, root: &Path, package_path: Option<PathBuf>) -> Self {
            let (library, fonts) = typst_globals();
            // Root the typst World at the *project* root so a spec in a
            // subdirectory can `#import "../sibling/helper.typ"` without
            // tripping VirtualPath's root-escape guard. The main module's
            // vpath is `source_path` relative to `root` (so a sibling file
            // with the same name is not shadowed by the in-memory main).
            // `VirtualPath::new` is `&str`-only and fallible (rejects `\` and
            // root-escaping paths). On Windows `strip_prefix` yields `\`
            // separators, so normalise to `/` first; fall back to the bare
            // file name, then a known-good vpath, if the converted path is
            // still rejected.
            let vpath = source_path
                .strip_prefix(root)
                .ok()
                .map(|p| p.to_string_lossy().replace('\\', "/"))
                .and_then(|s| VirtualPath::new(&s).ok())
                .or_else(|| {
                    source_path
                        .file_name()
                ...
                .unwrap_or_else(|| VirtualPath::new("main.typ").unwrap());
        ...
        fn read(&self, id: FileId) -> FileResult<Vec<u8>> {
            let root = match id.root() {
                VirtualRoot::Package(spec) => self.package_root(spec)?,
                VirtualRoot::Project => self.base_dir.clone(),
            };
            ...
            //
            // Record the *unresolved* path (no `canonicalize`): the watcher
            // strips `project_root` (or its canonical form) as a prefix, and
            // an interior-symlink-resolved path matches neither, so the dep
            // would silently drop.
            if matches!(id.root(), VirtualRoot::Project) {
                self.read_paths.lock().unwrap().insert(path.clone());
            }
    ...
    impl World for SpecWorld {
        ...
        fn file(&self, id: FileId) -> FileResult<Bytes> {
            if id == self.main.id() {
                return Ok(Bytes::new(self.main.text().as_bytes().to_vec()));
            }
    ...
    fn strip_tracey_imports(content: &str) -> std::borrow::Cow<'_, str> {
        ...
        while let Some(line) = lines.next() {
            ...
            // Slice off any trailing `// ...` line comment so a `(` inside it
            // doesn't masquerade as an unbalanced multi-line opener.
            let code = trimmed.split_once("//").map(|(c, _)| c).unwrap_or(trimmed).trim_end();
            if code.contains('(') && !code.ends_with(')') {
                // Cap the continuation scan and bail on any line that is
                // clearly not part of an import item list (`#...` or `= ...`).
                // A forgotten `)` would otherwise drain to EOF and silently
                // empty the spec; leaving the rest intact lets typst surface
                // the real "unclosed delimiter" error instead.
                for cont in lines.by_ref().take(32) {
                    let head = cont.trim_start();
                    ...
                    let c = cont.trim_end();
                    let c = c.split_once("//").map(|(h, _)| h).unwrap_or(c).trim_end();
                    if c.ends_with(')') {
                        break;
                    }
                }
    ...
    fn assign_heading_ids(
        ...
        // Cache the next absolute match positions and only re-search the one
        // that was consumed; the other stays valid across iterations. This
        // keeps the scan O(N) instead of re-scanning the tail on every
        // sentinel.
        let find_from = |from: usize, needle: &str| body[from..].find(needle).map(|p| from + p);
        let mut next_h = find_from(0, &h_needle);
        let mut next_r = find_from(0, &r_needle);
        ...
        loop {
                ...
                && next_h.is_none_or(|h| r < h)
            {
                ...
                if let Some(def) = body[r..tag_end]
                    ...
                    .and_then(|s| {
                        ...
                        v.find('"').map(|e| html_escape::decode_html_entities(&v[..e]))
                    })
                ...
                next_r = find_from(cursor, &r_needle);
                if next_h.is_some_and(|h| h < cursor) {
                    next_h = find_from(cursor, &h_needle);
                }
                continue;
            }
            ...
            // If `>` is somehow absent (malformed input), advance to the end
            // of `body` so the loop terminates instead of spinning.
            ...
            let title = tag
                ...
                .and_then(|s| {
                    ...
                    v.find('"').map(|e| html_escape::decode_html_entities(&v[..e]).into_owned())
                })
                .unwrap_or_default();
            let base = {
            ...
            cursor = tag_end;
            next_h = find_from(cursor, &h_needle);
            if next_r.is_some_and(|r| r < cursor) {
                next_r = find_from(cursor, &r_needle);
            }
    ...
    /// with `badge_for(def, source_path) …inner… REQ_CONTAINER_CLOSE`. The
    /// literal `X` is HTML-unescaped, parsed into a [`marq::RuleId`], and
    /// looked up in `by_id` (from the tree-sitter parse) so
    /// `data-req-id="a.b+1"` matches the version-1 definition keyed as `a.b`.
    /// If not found the sentinel wrapper is dropped and the inner body emitted
    /// verbatim. Nested `<div>`s inside the body are handled by depth-counting.
    ///
    /// Nested *sentinels* (a `#req` written inside another `#req`'s body) are
    /// stripped: typst evaluates the inner call before the outer body runs, so
    /// the prelude cannot suppress them, and a depth-counted `inner` slice
    /// would otherwise carry the raw `<div class="tracey-req" …>` into the
    /// final HTML. Only the outermost sentinel becomes a badge container.
    fn splice_req_badges(
        ...
        // Stack entry per open `<div>` we are currently *inside*.
        //   Some(true)  — top-level sentinel with badge: replace `</div>` with REQ_CONTAINER_CLOSE.
        //   Some(false) — sentinel wrapper dropped (nested, or id miss): swallow `</div>`.
        //   None        — plain `<div>` emitted verbatim from a prefix slice: re-emit `</div>`.
        // Tracking plain opens keeps a `</div>` that belongs to a plain div
        // (opened between an outer and a nested sentinel) from being mistaken
        // for the outer sentinel's close.
        let mut close_pending: Vec<Option<bool>> = Vec::new();
        // Record plain `<div>` opens in `slice` so their later `</div>`s pop a
        // `None` (re-emitted verbatim) instead of closing an enclosing sentinel
        // early. Called for every prefix slice emitted while inside a sentinel.
        fn track_plain_opens(slice: &str, stack: &mut Vec<Option<bool>>) {
            let bytes = slice.as_bytes();
            let mut i = 0;
            while let Some(off) = slice[i..].find("<div") {
                let p = i + off;
                if matches!(bytes.get(p + 4), Some(b' ' | b'>' | b'\t' | b'\n' | b'/')) {
                    stack.push(None);
                }
                i = p + 4;
            }
        ...
        let mut rest = body;
        loop {
            let next_open = rest.find(open_prefix.as_str());
            // While inside any tracked div, the next literal `</div>` competes
            // with the next sentinel open; plain-div depth is on the stack so
            // no separate matching walk is needed.
            let next_close =
                (!close_pending.is_empty()).then(|| rest.find("</div>")).flatten();

            match (next_open, next_close) {
                // Innermost tracked div closes before any further sentinel opens.
                (o, Some(c)) if o.is_none_or(|o| c < o) => {
                    // The slice up to the close may itself open plain divs
                    // (e.g. `body<div>n` before the first `</div>`); those
                    // must be on the stack so this `</div>` closes the
                    // innermost plain open, not the enclosing sentinel.
                    track_plain_opens(&rest[..c], &mut close_pending);
                    out.push_str(&rest[..c]);
                    match close_pending.pop().flatten() {
                        Some(true) => out.push_str(REQ_CONTAINER_CLOSE),
                        Some(false) => {}
                ...
                (Some(start), _) => {
                    ...
                    out.push_str(prefix);
                    // Only needed while already inside a sentinel; at top
                    // level plain divs and their closes pass through verbatim
                    // (next_close is None when the stack is empty).
                    if !close_pending.is_empty() {
                        track_plain_opens(prefix, &mut close_pending);
                    }
                    let after_prefix = &rest[start + open_prefix.len()..];
                    // ID runs to the next quote; typst html-escapes attribute
                    // values so a literal `"` cannot appear inside.
                    ...
                    let emit_close = if close_pending.is_empty() {
                        let id = html_escape::decode_html_entities(&after_prefix[..id_end]);
                        match marq::parse_rule_id(&id).and_then(|rid| by_id.get(&rid)) {
                            Some(def) => {
                                out.push_str(&badge_for(def, source_path));
                                Some(true)
                            }
                            // tree-sitter and the compiler disagree (e.g. user
                            // redefined `#req`); pass the body through unwrapped.
                            None => Some(false),
                        }
                    } else {
                        // Nested sentinel: drop the wrapper, keep the body.
                        Some(false)
                    };
                    close_pending.push(emit_close);
                    rest = &after_prefix[id_end + tag_end_rel + 1..];
                }
                // `(None, Some(_))` is fully handled by the guarded arm above
                // (`is_none_or` is always true for `None`); this arm only ever
                // fires for `(None, None)` but is widened for exhaustiveness.
                (None, _) => {
                    out.push_str(rest);
                    return out;
                }
    ...
    mod tests {
        ...
        /// Build a [`BadgeFn`] from a path-agnostic closure.
        fn badge(
            f: impl Fn(&marq::ReqDefinition) -> String + Send + Sync + 'static,
        ) -> BadgeFn {
            std::sync::Arc::new(move |d, _| f(d))
        }
        ...
        async fn render_bodyless_req() {
            let src = "#r(\"a.b\")\n";
            let b = badge(|d| format!("<OPEN {}>", d.id));
            let doc = render(src, Path::new("test.typ"), Path::new("."), None, &b, &mut alloc(), &mut nodeps())
                .await
        ...
        async fn render_with_unknown_prefix_fails_compile() {
            ...
            let b = badge(|d| format!("<OPEN {}>", d.id));
            let err = render(src, Path::new("test.typ"), Path::new("."), None, &b, &mut alloc(), &mut nodeps())
                .await
        ...
        async fn render_heading_slugs_from_sentinels() {
            ...
            let b = badge(|d| format!("<OPEN {}>", d.id));
            let doc = render(src, &dir.path().join("main.typ"), dir.path(), None, &b, &mut alloc(), &mut nodeps())
                .await
        ...
        async fn render_resolves_relative_import() {
            ...
            let b = badge(|d| format!("<OPEN {}>", d.id));
            let mut deps = nodeps();
            let doc = render(src, &dir.path().join("main.typ"), dir.path(), None, &b, &mut alloc(), &mut deps)
                .await
        ...
        async fn render_reports_dep_for_broken_helper() {
            ...
            let b = badge(|_| String::new());
            let mut deps = nodeps();
            let err = render(src, &dir.path().join("main.typ"), dir.path(), None, &b, &mut alloc(), &mut deps)
                .await
        ...
        async fn render_resolves_sibling_named_spec_typ() {
            ...
            let b = badge(|d| format!("<OPEN {}>", d.id));
            let doc = render(src, &dir.path().join("api.typ"), dir.path(), None, &b, &mut alloc(), &mut nodeps())
                .await
        ...
        async fn render_resolves_vendored_package() {
            ...
            let b = badge(|d| format!("<OPEN {}>", d.id));
            let mut deps = nodeps();
            let doc = render(src, Path::new("test.typ"), Path::new("."), Some(dir.path()), &b, &mut alloc(), &mut deps)
                .await
        ...
        fn splice_replaces_sentinel_div() {
            ...
            let out = splice_req_badges(body, &by_id, &b, "");
            assert_eq!(
                out,
                format!("<p>x</p><OPEN a.b>body<div>n</div>{REQ_CONTAINER_CLOSE}<p>y</p>")
            );
        }
        ...
        fn splice_normalises_versioned_id() {
            ...
            let by_id: HashMap<_, _> = [(def.id.clone(), &def)].into_iter().collect();
            let b = badge(|d| format!("<OPEN {}>", d.anchor_id));
            // Literal `+1` plus an HTML-escaped `&` in the base.
            let body = r#"<div class="tracey-req" data-req-id="a.b+1">body</div>"#;
            assert_eq!(
                splice_req_badges(body, &by_id, &b, ""),
                format!("<OPEN r--a.b>body{REQ_CONTAINER_CLOSE}")
            );
        }
        ...
        async fn render_req_with_explicit_version_suffix() {
            ...
            let b = badge(|d| format!("<div id=\"{}\">", d.anchor_id));
            let doc = render(src, Path::new("test.typ"), Path::new("."), None, &b, &mut alloc(), &mut nodeps())
                .await
        ...
        /// Nested `#req` must not emit an inner sentinel div: the prelude
        /// shadows `req`/`r` inside the body so only the outer id wraps.
        #[tokio::test]
        async fn render_nested_req_emits_single_sentinel() {
            let src = "#req(\"a\")[#req(\"b\")[inner]]\n";
            let b = badge(|d| format!("<div data-out=\"{}\">", d.id));
            let doc = render(src, Path::new("test.typ"), Path::new("."), None, &b, &mut alloc(), &mut nodeps())
                .await
                .expect("render nested req");
            // Exactly one badge container (outer "a"); no raw inner sentinel
            // survived into the spliced HTML.
            assert_eq!(doc.html.matches("data-out=\"a\"").count(), 1, "{}", doc.html);
            assert!(!doc.html.contains("tracey-req"), "{}", doc.html);
            assert!(!doc.html.contains("data-req-id"), "{}", doc.html);
            assert!(doc.html.contains("inner"), "{}", doc.html);
        }
    }
}
...
fn diff_inline(old: &str, new: &str) -> Option<String> {
    ...
    let m = old_words.len();
    let n = new_words.len();
    // cap LCS table at ~16MB to avoid OOM on pathological rule bodies
    if (m + 1).saturating_mul(n + 1) > 4_000_000 {
        return None;
    }
...
fn extract_marker_prefix(content: &str, span: SourceSpan) -> Option<String> {
    ...
    // "req" is the long alias for "r"; normalize so prefix-matching against source refs works
    let prefix = if prefix == "req" { "r" } else { prefix };
...
"#;
    ...
    fn extracts_prefix() {
        ...
        // `#req` normalizes to `r` so it matches source-ref prefixes.
...
"#;
    ...
    #[tokio::test]
    async fn marker_extraction_is_allowlist() {
        // Only `#r(..)` and `#req(..)` are requirement markers. Everything
        // else — typst stdlib (`#image`, `#link`), third-party packages
        // (unify's `#qty`, glossarium's `#gls`), or arbitrary user-defined
        // functions — must be ignored even though it matches the
        // `#ident("str")[body]` call shape.
        let src = concat!(
            "#image(\"foo.png\")\n",
        ...
        let doc = parse(src).await.unwrap();
        let ids: Vec<_> = doc.reqs.iter().map(|r| r.id.base.as_str()).collect();
